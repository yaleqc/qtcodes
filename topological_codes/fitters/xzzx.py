...
class XZZXGraphDecoderBase(TopologicalGraphDecoder[TQubit]):
    ...
    def _specify_virtual(self) -> Dict[str, List[TQubit]]:
        """
        Define coordinates of B and A virtual syndrome nodes. Our convention is that B
        virtual syndrome nodes are top/bottom and A virtual nodes are left/right.
        Args:
        Returns:
            virtual (Dict[str, List[TQubit]]): 
                where virtual["A"] holds a list of tuples specifying virtual A syndrome nodes
                and equivalently for virtual["B"]
        """
        virtual: Dict[str, List[TQubit]] = {}
        virtual["A"] = []
        virtual["B"] = []
        for j in range(0, self.code_params["d"], 2):
            # Z virtual nodes
            virtual["B"].append((-1, -0.5, j - 0.5))  # top
            virtual["B"].append((-1, self.code_params["d"] - 0.5, j + 0.5))  # bottom

            # X virtual nodes
            virtual["A"].append((-1, j + 0.5, -0.5))  # left
